...
export const createElectionReducer = (state, { type, payload }) => {
  switch (type) {
    case CREATE_ELECTION_ACTIONS.ELECTION_TITLE:
      return { ...state, electionTitle: payload, error: false };

    case CREATE_ELECTION_ACTIONS.ELECTION_DESCRIPTION:
      return { ...state, electionDescription: payload, error: false };
    ...
    case CREATE_ELECTION_ACTIONS.WEIGHTED_VOTING_ACTIVE:
      return {
        ...state,
        weightedVoting: {
          active: !state.weightedVoting.active,
    ...
    case CREATE_ELECTION_ACTIONS.SUBTITLE: {
      const newQuestions = state.questions.map((question, index) => {
        if (index !== payload.index) return question;
        return {
          ...question,
